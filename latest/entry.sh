...
# Sourced BEFORE the s3fs pass so env files can carry AWS/endpoint variables.
...
# tini restores the base image's PID-1 reaper (our ENTRYPOINT replaces it);
# -g signals the whole process group so kernels, rserver, shiny, code-server,
# and npx-spawned MCP servers all get SIGTERM when VICE stops the pod.
exec /usr/bin/tini -g -- jupyter lab --no-browser --LabApp.token="" --LabApp.password="" --ip="0.0.0.0" --port=8888
